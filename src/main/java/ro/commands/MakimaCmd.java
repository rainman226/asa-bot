...
public class MakimaCmd extends Command {
    ...
    private final int MAX_LIMIT = 3393;

    public MakimaCmd() {
        this.name = "makima";
        this.arguments = "<tag>";
        this.help = "Get pictures of Makima";
    ...
    protected void execute(CommandEvent event) {
        ...
        try {
            if(event.getArgs().isEmpty()) {
                safeImgBoard.search(MAX_LIMIT, "makima_(chainsaw_man) " + event.getArgs()).async(images -> {
                    event.reply(images.get(new Random().nextInt(images.size())).getURL());
                });
            } else {
                imgBoard.search(MAX_LIMIT, "makima_(chainsaw_man) " + event.getArgs()).async(images -> {
                    event.reply(images.get(new Random().nextInt(images.size())).getURL());
                });
            }
        } catch (Exception e) {
